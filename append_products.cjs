...
// Append new products to existing array string
const newProductsStr = ',\n' + JSON.stringify(newProducts, null, 2).replace(/"([^"]+)":/g, '$1:').slice(1, -1) + '\n];';
const updatedContent = fileContent.replace(/\n\];$/, newProductsStr);

fs.writeFileSync('src/data/products.js', updatedContent);
console.log(`Successfully added ${newProducts.length} new products to the catalog.`);
